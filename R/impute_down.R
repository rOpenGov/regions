...
impute_down <- function( upstream_data = NULL,
  ...
  if ( values_var %in% names(upstream_df) ) {
    names(upstream_df)[which(names(upstream_df)==values_var)] <- 'impute_values'
  } else {
  ...
  if ( ! is.null(upstream_method_var) ) {   ## upstream method name is given 
    if ( upstream_method_var %in% names(upstream_df) ) {
      names(upstream_df)[which(names(upstream_df)==upstream_method_var)] <- 'impute_method'
    } else {
    ...
    if ( any ( names(upstream_df) %in% possible_method_vars ) ) {
      ...
      warning("The upstream_df has a variable called '", conflicting_variable_name , 
              "' that will be changed to\n'", new_variable_name, "' to avoid confusion.", 
  ...
  if ( country_var %in% names(downstream_df) ) {
    names(downstream_df)[which(names(downstream_df)==country_var)] <- 'country_code'
  } else {
    stop ( "The 'country_var' is not found in the downstream data.")
  }

  if ( regional_code %in% names(downstream_df) ) {
    names(downstream_df)[which(names(downstream_df)==regional_code)] <- 'geo_code'
  } else {
  ...
  if ( ! is.null(downstream_method_var) ) {
    if ( downstream_method_var %in% names(downstream_df) ) {
      names(downstream_method_df)[which(names(downstream_df)==downstream_method_var)] <- 'method'
    } else {
