upstream1 <- data.frame ( country_code =  rep( "AU", 3),
                         year = c(2018:2020),
                         my_var  = c(10,12,11),
...
upstream2 <- data.frame ( country_code =  c(rep( "AU", 2),
                                            rep( "DE", 2)),
...
impute2 <- impute_down (upstream_data  = upstream2,
...
test_that("warning works", {
  expect_warning(impute_down (upstream_data  = upstream2,
                              downstream_data = downstream2,
                              time_var = NULL,
                              country_var = "country_code",
                              regional_code = "geo_code",
...
impute3 <- impute_down (upstream_data  = upstream2,
...
upstream4 <- data.frame ( country_code =  c(rep( "AU", 2),
                                            rep( "DE", 2)),
                          year = rep(c(2018:2019),2),
...
impute4 <- impute_down (upstream_data  = upstream4,
                        downstream_data = downstream3,
                        time_var = 'year',
                        country_var = "country_code",
                        regional_code = "geo_code",
...
test_that("impute_down gives correct methods", {
  expect_equal(impute4$method, c('missing', 'actual', 
                                 'actual', 'imputed from DE actual', 
                                 'imputed from AU actual', 'actual',
                                 'actual', 'imputed from DE actual') )
  expect_equal(all(c("geo_code", "country_code", "values", "method", 
                     "description", "geo_name", "year") %in% names( impute2)), 
               TRUE)
  expect_equal(any(c("method.y", "method.x", "values.x", "year.x") %in% names( impute2)), 
               FALSE)
