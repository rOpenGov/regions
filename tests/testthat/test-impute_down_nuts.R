...
tested_imputation1 <- impute_down_nuts(
  dat = dat,
  geo_var = "geo",
  values_var = "values",
  method_var = "method",
...
luxembourg <-
  tested_imputation1[substr (tested_imputation1$geo, 1, 2) == "LU", ]

tested_imputation_mt <-
  impute_down_nuts(dat = data.frame (geo = "MT",
                                     values = 12))

test_that("all units present", {
  expect_equal(luxembourg$geo,
               c("LU", "LU0", "LU00", "LU000",
                 "LUZ", "LUZZ", "LUZZZ"))
  expect_equal(
    tested_imputation_mt$geo,
    c("MT", "MT0", "MT00", "MT001",
